...
class Loggers {
  ...
  init() {
    ...
    const requestLogStream = createWriteStream(
      resolve(__dirname, "../../logs/request.log"),
      {
        flags: "a",
        // eslint-disable-next-line object-curly-newline
      }
    ...
    this.morganRequestMiddleware = morgan(morganFormat, {
    ...
    const logTransports = {
      ...
      combinedLog: new transports.File({
        level: "info",
        filename: "logs/combined.log",
      }),
      errorLog: new transports.File({
        level: "error",
    ...
    this.logger = createLogger({
      transports: [
        ...
        logTransports.combinedLog,
        logTransports.errorLog,
      ],
  ...
  async retrieveLogs({
    ...
    try {
      ...
      const logs = await readFiles(
        resolve(__dirname, `../../logs/${type}.log`),
        {
          encoding: "utf8",
        }
      );
      ...
      !convertedLogsToArray[convertedLogsToArray.length - 1] &&
        convertedLogsToArray.pop();
  ...
  static formatString(type) {
    return (previous, current) => {
      ...
      if (type === "request") {
        const { timestamp, ip, method, route, version, status, runtime } =
          current;
        return (
          previous +
